...
def matmul_add(B, N, L, M, dtype):
    A = te.placeholder((B, N, L), name="A", dtype=dtype)
    B = te.placeholder((B, M, L), name="B", dtype=dtype)
    # C = te.placeholder((N, M), name="C", dtype=dtype)
    matmul = topi.nn.bath_matmul(A, B)
...
B = 8
N = L = M = 1024
task = tvm.auto_scheduler.SearchTask(func=matmul_add, args=(B, N, L, M, "float32"), target=target)
