...
import torchprof
x = torch.rand([1, 3, 800, 1066]).cuda()
with torchprof.Profile(detr, use_cuda=True) as prof:
    detr(x)
...
def detect(im, model, transform):
    ...
    for i in range(20):
        outputs = model(img)
    print("time cost:", (time.time() - start)*1000/20,"ms")
    print("FPS:", 1 / (time.time()-start)*20)
